...
class LeCartWoocommerce(LeCartWordpress):
	...
	def category_import(self, convert, category, categories_ext):
		category_term = {
			'name': convert['name'],
			'slug': convert['name'].replace(" ","-").lower(),
			'term_group': 0,
		}
		category_term_query = self.create_insert_query_connector('terms', category_term)
		term_id = self.import_page_data_connector(category_term_query, True, convert['id'])
		if not term_id:
		...
		category_taxonomy = {
			...
			'description':  '',
			'parent': convert['parent'] if convert['parent'] else 0,
			'count': convert['sort_order']
		}
		category_taxonomy_query = self.create_insert_query_connector('term_taxonomy', category_taxonomy)
		category_term_import = self.import_page_data_connector(category_taxonomy_query, True, convert['id'])
		if not category_term_import:
			return response_warning('category ' + to_str(convert['id']) + ' import false.')
		self.insert_map(self.TYPE_CATEGORY if not self.blog_running else self.TYPE_CATEGORY_BLOG, convert['id'], category_term_import, to_str(convert['code']), term_id)
	...
	def check_product_import(self, convert, product, products_ext):
		...
		for attribute in convert['attributes']:
			att_name = self.get_map_field_by_src(self.TYPE_ATTR, attribute['attribute_id'],None, None, 'code_src')
			if attribute['attribute_name'] != att_name:
				product_attr = {
				'attribute_name': attribute['attribute_name'].lower(),
				'attribute_label': attribute['attribute_name'],
				'attribute_type': 'select',
				'attribute_orderby':'menu_order',
				'attribute_public': 0,
				}
				attr_query = self.create_insert_query_connector('woocommerce_attribute_taxonomies', product_attr)
				attr_id = self.import_product_data_connector(attr_query, True, convert['id'])
				if not attr_id:
					return response_warning('attribute ' + to_str(convert['id']) + ' import false.')
				self.insert_map(self.TYPE_ATTR, attribute['attribute_id'], attr_id, attribute['attribute_name'])

			for attribute_value in convert['attributes_value']:
				if attribute_value['attribute_id'] == attribute['attribute_id']:
					check_term_id = self.get_map_field_by_src(self.TYPE_ATTR_VALUE, None, attribute_value['value'], None, 'id_desc')
					if not check_term_id:
						term_data = {
							'name': attribute_value['value'],
							'slug': attribute_value['value'].lower(),
							'term_group': 0,
						}
						term_query =  self.create_insert_query_connector("terms", term_data)
						term_id = self.import_product_data_connector(term_query, True, convert['id'])
						if not term_id:
							return response_warning('attribute ' + to_str(convert['id']) + ' import false.')
						self.insert_map(self.TYPE_ATTR_VALUE, attribute_value['attribute_id'], term_id, attribute_value['value'])
						term_meta = {
							'term_id': term_id,
							'meta_key': 'order_pa_' + attribute['attribute_code'],
							'meta_value': 0
						}
						term_meta_query = self.create_insert_query_connector('termmeta', term_meta)
						self.import_product_data_connector(term_meta_query, True, convert['id'])
						term_taxonomy = {
							'term_id': term_id,
						...
						term_taxonomy_id = self.import_product_data_connector(term_taxonomy_query, True, convert['id'])
						self.insert_map('attr_term_taxonomy', attribute_value['attribute_id'], term_taxonomy_id,
		...
		for attribute_value in convert['attributes_value']:
			...
			product_id = self.get_map_field_by_src(self.TYPE_PRODUCT, attribute_value['product_id'])
			term_taxonomy_id = self.get_map_field_by_src('attr_term_taxonomy', None, attribute_value['value'], None)
			attr_name = self.get_map_field_by_src(self.TYPE_ATTR, attribute_value['attribute_id'],None, None, 'code_src')
			relationships_data = {
				'object_id': product_id,
				'term_taxonomy_id': term_taxonomy_id,
				'term_order': 0
			}
			relationships_query = self.create_insert_query_connector('term_relationships', relationships_data)
			self.import_product_data_connector(relationships_query, True, convert['id'])
			meta_data['pa_' + attr_name.lower()] = {
				'name': 'pa_' + attr_name.lower(),
				'value': '',
				'position': position,
				'is_visible': 1,
				'is_variation': 0,
			...
			position += 1
		update_attr_data = {
			'meta_value': php_serialize(meta_data)
		}
		where_attr_update = {
			'post_id': product_id,
		...
		update_product_id = self.import_data_connector(attribute_update_query, self.TYPE_PRODUCT, product_id)
		if not update_product_id:
			return response_warning('Can not update product id:' +  update_product_id)
		return self.get_map_field_by_src(self.TYPE_PRODUCT, convert['id'], convert['code'], lang = self._notice['target']['language_default'])
	...
	def product_import(self, convert, product, products_ext):
		...
		post_query = self.create_insert_query_connector('posts', data)
		product_id = self.import_product_data_connector(post_query, True, convert['id'])
		#self.import_page_data_connector(metapost_query, True, product_id)
		if not product_id:
	...
	def after_product_import(self, product_id, convert, product, products_ext):
		...
		product_meta = {
			               '_edit_lock': '',
			               '_thumbnail_id': thumbnail_id if thumbnail_id != False else '',
			               '_sku': convert['sku'],
			               '_regular_price': convert['price'],
			               ...
			               '_download_limit': -1,
			               '_download_expiry': -1,
			               '_stock': convert['qty'] if int(convert['qty']) > 0 else 0,
			               '_stock_status': 'instock' if convert['is_in_stock'] == True else 'outofstock',
						   '_wc_average_rating': 0,
			               '_wc_review_count': 0,
			               '_product_version': '5.1.0',
		...
		if convert['categories']:
			...
			for term in all_terms:
				...
				category_query = self.create_insert_query_connector("term_relationships", category_data)
				self.import_product_data_connector(category_query, True, convert['id'])
	...
	def customer_import(self, convert, customer, customers_ext):
		data = {
			...
			'user_activation_key': '',
			'user_status': 1 if convert['active'] == True else 0,
			'display_name': convert['first_name'] + convert['last_name'],
		}
		user_query = self.create_insert_query_connector("users", data)
		user_id = self.import_customer_data_connector(user_query, True, convert['id'])
		if not user_id:
			return response_error('User ' + to_str(convert['id']) + ' import false.')
		self.insert_map(self.TYPE_CUSTOMER, convert['id'], user_id, convert['last_name'])
	...
	def after_customer_import(self, customer_id, convert, customer, customers_ext):
		...
		#'order_count': 0,
		#'billing_first_name':duck,
		#'billing_last_name':donal,
		#'billing_company' :disney,
		#'billing_address_1': '',
		#'billing_address_2':'',
		#'billing_city': '',
		...
		#'billing_country':S,
		#'billing_state':'',
		#'billing_phone':12345674,
		#'billing_email':'dnaldd @ quackquack.com',
		#'shipping_first_name':'',
		#'shipping_last_name':'',
		#'shipping_company':'',
		#'shipping_address_1':'',
	...
	def order_import(self, convert, order, orders_ext):
		...
		order_data ={
			...
			'post_content':	'',
			'post_title': 'Order &ndash' + to_str(convert['created_at']),	
			'post_excerpt':	'',
			'post_status':	order_status, #convert['status'],
			'comment_status': 'closed',
			'ping_status':	'closed',
			'post_password':'',
			'post_name':	'order-'+to_str(convert['created_at']), 
			'to_ping':	'',
			'pinged':	'',
		...
		order_id = self.import_customer_data_connector(order_query, True, convert['id'])
		if not order_id:
			return response_error('Order ' + to_str(convert['id']) + ' import false.')
	...
	def after_order_import(self, order_id, convert, order, orders_ext):
		...
		first_name_bill = convert['billing_address']['first_name']
		last_name_bill = convert['billing_address']['last_name']
		first_name_ship = convert['shipping_address']['first_name']
		last_name_ship = convert['shipping_address']['last_name']

		email = convert['customer']['email'] if convert['customer']['email'] else ''
		telephone = convert['customer_address']['telephone'] if convert['customer_address']['telephone'] else ''
		...
		data_postmeta = {
			...
			'_order_tax': '0',
			'_order_shipping': convert['shipping']['amount'],  # c
			'_order_total': convert['total']['amount'],  # c
			'_order_version': '5.1.0',
			'_prices_include_tax': 'no',
			'_billing_address_index': first_name_bill + ' ' + last_name_bill + ' ' +
									  ...
									  convert['billing_address']['country']['name'] + ' ' +
									  convert['billing_address']['country']['code'] + ' ' +
									  convert['billing_address']['state']['name'] + ' ' +
									  email + ' ' + telephone,
			'_shipping_address_index': first_name_ship + ' ' + last_name_ship + ' ' +
									   ...
									   convert['shipping_address']['country']['code'] + ' ' +
									   convert['shipping_address']['state']['name'] + ' ' +
									   email + ' ' + telephone,
			'_cart_discount': '0',
			...
			'_transaction_id': '',
			'_created_via': 'admin',
			# '_date_completed': '1616637500',
			# '_date_paid': '1616637500',
			'_billing_first_name': first_name_bill,
			'_billing_last_name': last_name_bill,
			...
			'_billing_state': convert['billing_address']['country']['name'],
			'_billing_postcode': convert['billing_address']['country']['code'],
			'_billing_country': convert['billing_address']['state']['name'],
			'_billing_email': email,
			'_billing_phone': telephone,
			'_shipping_first_name': first_name_ship,
			'_shipping_last_name': last_name_ship,
			...
			'_shipping_state': convert['shipping_address']['country']['name'],
			'_shipping_postcode': convert['shipping_address']['country']['code'],
			'_shipping_country': convert['shipping_address']['state']['name'],
			'_paid_date': get_current_time(),
			'_completed_date': get_current_time(),
		...
		for key, value in data_postmeta.items():
			postmeta = {
				'post_id': order_id,
				'meta_key': key,
				'meta_value': value,
			}
			order_postmeta_query = self.create_insert_query_connector("postmeta", postmeta)
			order_postmeta_id = self.import_order_data_connector(order_postmeta_query, True, convert['id'])
			if not order_postmeta_id:
				return response_warning('order_postmeta ' + to_str(convert['id']) + ' import false.')

		# woocommerce_order_item:

		data_orders_src = self.select_map(self._migration_id, self.TYPE_ORDER, None, order_id)
		# id_orders_src = data_orders_src['id_src']
		data_product = get_list_from_list_by_field(convert['items'], 'id', convert['id'])
		amount_product = 0
		shipping_and_fee = 0
		for product in data_product:
			amount_product += to_decimal(product['qty'])
			# (woocommerce_order_items)
			if shipping_and_fee == 0:
				order_items_data = {
					'line_item': product['product']['name'],
					'shipping': 'Shipping',
					'fee': '£' + to_str(convert['tax']['amount']) + ' fee'
				}
			else:
				order_items_data = {
					'line_item': product['product']['name'],
			...
			for key, value in order_items_data.items():
				order_items = {
					'order_item_name': value,
					'order_item_type': key,
					'order_id': order_id,
				}
				order_items_query = self.create_insert_query_connector("woocommerce_order_items", order_items)
				order_items_id = self.import_order_data_connector(order_items_query, True, convert['id'])
				if not order_items_id:
					return response_warning('order_items ' + to_str(convert['id']) + ' import false.')

				if key == 'line_item':
					# (woocommerce_order_itemmeta)
					order_item_meta_1 = {
						'_product_id': order_id,
						'_variation_id': 0,
						'_qty': product['qty'],
						'_tax_class': '',
						'_line_subtotal': to_decimal(product['original_price']) * to_decimal(product['qty']),
						'_line_subtotal_tax': 0,
						'_line_total': to_decimal(product['original_price']) * to_decimal(product['qty']),
						'_line_tax': 0,
						'_line_tax_data': 'a:2:{s:8:"subtotal";a:0:{}s:5:"total";a:0:{}}',
						'_reduced_stock': product['qty'],
					}
					for key, value in order_item_meta_1.items():
						order_item_meta_data = {
							'order_item_id': order_items_id,
							'meta_key': key,
							'meta_value': value,
						}
						order_items_meta_query = self.create_insert_query_connector("woocommerce_order_itemmeta",
																					order_item_meta_data)
						order_items_meta_id = self.import_order_data_connector(order_items_meta_query, True,
																			   convert['id'])
						if not order_items_meta_id:
							return response_warning('order_items ' + to_str(convert['id']) + ' import false.')
					...
					order_product_lookup_data = {
						...
						'variation_id': 0,
						'customer_id': convert['customer']['id'] if convert['customer']['id'] else 0,
						'date_created': convert['created_at'] if convert['created_at'] else convert['updated_at'],
						'product_qty': product['qty'],
						'product_net_revenue': to_decimal(product['original_price']) * to_decimal(product['qty']),
						'product_gross_revenue': to_decimal(convert['shipping']['amount']) + to_decimal(
							product['original_price']) * to_decimal(product['qty']),
						'coupon_amount': 0,
						'tax_amount': 0,
						'shipping_amount': convert['shipping']['amount'],
						'shipping_tax_amount': 0,
					}
					order_product_lookup_query = self.create_insert_query_connector('wc_order_product_lookup',
																					order_product_lookup_data)
					order_product_lookup_id = self.import_data_connector(order_product_lookup_query, self.TYPE_ORDER,
																		 order_id)
					if not order_product_lookup_id:
						return response_warning(
							'order id ' + to_str(order_id) + ' import to order_product_lookup fails!')

				if key == 'fee':
					order_item_meta_2 = {
						'_fee_amount': convert['tax']['amount'] if convert['tax']['amount'] else 0,
						'_tax_class': '',
						'_tax_status': 'taxable',
						'_line_total': convert['tax']['amount'] if convert['tax']['amount'] else 0,
						'_line_tax': 0,
						'_line_tax_data': 'a:1:{s:5:"total";a:0:{}}',
					}
					for key, value in order_item_meta_2.items():
						order_item_meta_data = {
							'order_item_id': order_items_id,
							'meta_key': key,
							'meta_value': value,
				...
				if key == 'shipping':
					...
					for key, value in order_item_meta_3.items():
						order_item_meta_data = {
							'order_item_id': order_items_id,
							'meta_key': key,
							'meta_value': value,
		...
		data_order_stats = {
			...
			'num_items_sold': amount_product,
			'total_sales': convert['total']['amount'],
			'tax_total': 0,
			'shipping_total': convert['shipping']['amount'],
			'net_total': convert['tax']['amount'],
			'returning_customer': 0,
			'status': 'wc-completed',
			'customer_id': 0,
		}
		order_data_order_stats = self.create_insert_query_connector("wc_order_stats", data_order_stats)
		order_data_order_id = self.import_order_data_connector(order_data_order_stats, True, convert['id'])
		if not order_data_order_id:
			return response_warning('wc_order_stats ' + to_str(convert['id']) + ' import false.')
		return response_success()
	...
	def page_import(self, convert, page, pages_ext):
		...
		self.insert_map(self.TYPE_PRODUCT, convert['id'], page_id, convert['title'], code_name, None, language_code)
