...
class LeCartCustom(LeBasecart):
	...
	def get_categories_main_export(self):
		...
		query = {
			...
			WHERE categories.categories_id > """ + to_str(
				id_src) + " ORDER BY categories.categories_id ASC LIMIT " + to_str(limit)
		}
	...
	def get_categories_ext_export(self, categories):
		...
		categories_ext_queries = {
			...
			'URIs': {
				'type': 'select',
				'query': "SELECT * FROM categories WHERE categories_id IN " + self.list_to_in_condition(category_ids)
			},
			'cate_imgs': {
				'type': 'select',
				'query': "SELECT categories_image FROM  categories WHERE categories_id IN " + self.list_to_in_condition(category_ids)
			},
		}
	...
	def convert_category_export(self, category, categories_ext):
		...
		#category_data['language']['id'] = category_description['language'] fix sau
	...
	def get_products_ext_export(self, products):
		...
		category_ids = duplicate_field_value_from_list(product_ext['data']['products_to_categories'], 'categories_id')
		category_id_con = self.list_to_in_condition(category_ids)
		product_ext_rel_queries = {
			'categories_description': {
				'type': 'select',
				'query': "SELECT * FROM `_DBPRF_categories_description` WHERE categories_id IN " + category_id_con
			},
			'attributes': {
				'type': 'select',
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data['id'] = product['products_id'] #fix product['id']
		...
		product_data['updated_at'] = product['products_last_modified'] if to_int(product['products_last_modified']) else product_data['created_at']
		...
		# product_data['tags'] = product_description_def['tag']
		url_product_image = 'http://localhost/customcart/image/'
		...
		# product_images = get_list_from_list_by_field(products_ext_data['products_images'], 'products_id',product['products_id'])
		# if product_images:
		# 	if check_thumbnail:
		...
		# attributes
		product_data['attributes'] = products_ext_data['attributes']

		# product_attribute
		product_att = get_list_from_list_by_field(products_ext_data['product_attribute'], 'product_id', product['products_id'])
		product_data['attributes_value'] = product_att
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		if address_books:
			for address_book in address_books:
				...
				country = get_row_from_list_by_field(customers_ext['data']['countries'], 'customers_id',address_book['entry_country_id'])
				if country:
					address_data['country']['id'] = country['country_id']
					address_data['country']['country_code'] = country['country_iso_code_2']
					address_data['country']['name'] = country['country_name']
				else:
	...
	def convert_order_export(self, order, orders_ext):
		...
		order_rows_id = get_list_from_list_by_list_field(orders_ext['data']['orders_total'], 'orders_id',
														 order['orders_id'])
		...
		order_data['tax']['amount'] = get_value_by_key_in_dict(ot_tax, 'value', 0.0)
		...
		order_customer['email'] = order['customers_email'] if order['customers_email'] != None else ''
		# customer_name = order['ship_fname'] + ' ' + order['ship_lname']
		if order['delivery_name']:
			order_customer['first_name'] = order['delivery_name'].split(' ')[0] if order['delivery_name'].split(' ')[
				0] else ''
			if len(order['delivery_name'].split(' ')) > 1:
				order_customer['last_name'] = order['delivery_name'].split(' ')[1] if order['delivery_name'].split(' ')[
					1] else ''
			else:
		...
		customer_address['telephone'] = order['customers_telephone'] if order['customers_telephone'] else ''
		customer_address['company'] = order['billing_company']
		...
		if customer_country:
			...
			customer_address['country']['country_code'] = customer_country['countries_iso_code_2'] if customer_country[
				'countries_iso_code_2'] else customer_country['countries_iso_code_3']
		...
		order_billing['first_name'] = order['billing_name'].split(' ')[0] if order['billing_name'].split(' ')[0] else ''
		order_billing['last_name'] = order['billing_name'].split(' ')[1] if order['billing_name'].split(' ')[1] else ''
		order_billing['address_1'] = order['billing_address1']
		order_billing['address_2'] = order['billing_address2'] if order['billing_address2'] else ''
		...
		order_delivery['first_name'] = order['delivery_name'].split(' ')[0] if order['delivery_name'].split(' ')[
			0] else ''
		order_delivery['last_name'] = order['delivery_name'].split(' ')[1] if order['delivery_name'].split(' ')[
			1] else ''
		order_delivery['address_1'] = order['delivery_address1']
		order_delivery['address_2'] = order['delivery_address2'] if order['delivery_address2'] else ''
		...
		delivery_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_name',
		...
		if delivery_state:
			order_delivery['state']['id'] = delivery_state['zone_code']
	...
	def get_reviews_ext_export(self, reviews):
		...
		reviews_ext_queries = {
			'products_description': {
				'type': 'select',
				'query': "SELECT products_id, language_id, products_name FROM products_description WHERE products_id IN " + self.list_to_in_condition(
					product_ids)
			},
			'reviews_description': {
				'type': 'select',
